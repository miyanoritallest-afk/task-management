...
| ソートボタン | 「↑ 優先度順」「↑ 期限順」ボタンをカラムヘッダー下段に配置。押下でカードを並び替え。再押下でソート解除。アクティブ状態は青でハイライト |
...
|------|------|
| トリガー | 「+ カードを追加」ボタンを押す |
| 入力UI | カラム末尾にテキスト入力欄・優先度選択・追加ボタンをインラインで表示 |
| 優先度選択 | 「🔴 高」「🟡 中」「🟢 低」「— なし」の4択ボタン。デフォルトは「なし」 |
| 追加ボタン | 入力欄が空でない場合のみ活性化。押下またはEnterキーでカードをカラム末尾に追加 |
...
| 要素 | 仕様 |
|------|------|
| トリガー | カードのタイトル部分をダブルクリック |
| 表示形式 | タイトル部分がテキスト入力欄に変わる（モーダル不使用） |
| 確定 | Enterキーまたはフォーカスアウトで保存 |
| キャンセル | Escキーで変更を破棄し元のタイトルに戻る |
| バリデーション | タイトルが空になった場合は変更を破棄 |
...
stateDiagram-v2
    ...
    カード追加入力欄 --> ボード画面 : 追加ボタン押下 / Enter（カード追加・DB保存）
    カード追加入力欄 --> ボード画面 : Esc / × ボタン（キャンセル）

    ボード画面 --> タイトルインライン編集 : カードタイトルをダブルクリック
    タイトルインライン編集 --> ボード画面 : Enter / フォーカスアウト（保存）
    タイトルインライン編集 --> ボード画面 : Esc（変更を破棄）

    ボード画面 --> カード詳細モーダル : カードタイトルをシングルクリック（フェーズ2）
